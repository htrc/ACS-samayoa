...
def run_spark(args, spark: SparkSession):
    ...
    num_topics = [2, 4, 6, 8, 10, 14, 18, 22, 26, 30, 35, 40, 50]
    ...
    features_df = vocab_model.transform(tokenized_df).cache()

    features_df.show()

    with open(os.path.join(args.output, 'k.csv'), 'w', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['k', 'log_likelihood', 'log_perplexity'])

        for k in num_topics:
            logger.info(f"Running LDA (k={k})...")
            lda = LDA(k=k)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-sk', '--start-k', required=True, type=int, metavar='NUM_TOPICS_START',
    #                     help='The lower bound on the number of topics to ask LDA to produce')
    # parser.add_argument('-ek', '--end-k', required=True, type=int, metavar='NUM_TOPICS_END',
    #                     help='The upper bound on the number of topics to ask LDA to produce')
    # parser.add_argument('-s', '--step', required=True, type=int, metavar='NUM_TOPICS_STEP',
    #                     help='The topic step')
